<?php
    ...
    function login($data){
        ...
        if( mysqli_num_rows($result) === 1 ){
            ...
            // Cek password yang dimasukkan apakah sama dengan di database
            $datas = mysqli_fetch_assoc( $result );
            $resultCheck = password_verify( $password, $datas['password'] );
            if ($resultCheck){
                
                // Tambah SESSION ketika berhasil login
                session_start();
                $_SESSION['user'] = true;

                // Jika remember me diaktifkan, buat COOKIE
                if( $rememberme ){
                    $hashedUsername = hash( 'sha512', $username );
                    setcookie( 'id', $datas['id'], time() + 60 );
                    setcookie( 'hashedUsername', $hashedUsername, time()+60 );
                }
    ...
    // COOKIE
    // Menambahkan input remember me di halaman login
    // Jika remember me aktif, buat COOKIE untuk id, dan hashedUsername
    // Cek apakah halaman login memiliki COOKIE login?
    // Jika memiliki COOKIE, maka ambil data dari database dan simpan username berdasarkan id COOKIE
    // Bandingkan COOKIE login dengan hashedUsername dari database
    // Jika benar, arahkan untuk ke halaman index (otomatis login)
    // Hapus COOKIE ketika user sengaja LOGOUT
